...
try {
    ...
    for ($i=0; $i -lt 45; $i++) {
        ...
        try { Invoke-WebRequest -UseBasicParsing -Uri 'http://localhost:8080/api/auth/login' -Method Options -TimeoutSec 2 | Out-Null; $ready=$true; break } catch { if ($_.Exception.Response) { $ready=$true; break } }
    ...
    $login=Invoke-Json 'http://localhost:8080/api/auth/login' '{"email":"admin@demo.regula.local","password":"RegulaDemo2026!"}'
    ...
    foreach ($case in @(@('100','IDENTIDAD'),@('200','SANCIONES'),@('300','PEP'))) {
        $response=Invoke-Json 'http://localhost:8080/api/kyc/consultar' (@{identificadorDocumento=$case[0];tipoConsulta=$case[1]} | ConvertTo-Json -Compress) $auth
        if ($response.Status -ne 200) { throw "KYC $($case[1]) falló con HTTP $($response.Status)." }
    }
    foreach ($document in 'not-found','rate-limit','server-error','unavailable','invalid-json') {
        ...
        $response=Invoke-Json 'http://localhost:8080/api/kyc/consultar' (@{identificadorDocumento=$document;tipoConsulta='IDENTIDAD'} | ConvertTo-Json -Compress) $auth
        if ($response.Status -eq 200) { throw "El escenario $document debía fallar controladamente." }
    }
    Start-Sleep -Seconds 3
    $watch=[Diagnostics.Stopwatch]::StartNew()
    $timeout=Invoke-Json 'http://localhost:8080/api/kyc/consultar' '{"identificadorDocumento":"timeout","tipoConsulta":"IDENTIDAD"}' $auth
    $watch.Stop()
